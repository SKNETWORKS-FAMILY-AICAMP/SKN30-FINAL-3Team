...
- [review engine](scripts/pr-policy-review.mjs): GitHub API로 PR metadata·patch와 허용된 정책·설정 파일의 제한된 head 내용을 읽고 Responses API 부분 리뷰를 병렬 실행한 뒤 결과를 통합하며 sticky comment, Check와 Discord 알림을 갱신한다.
...
- chunk가 하나면 Responses API를 한 번 호출한다. 둘 이상이면 기본 동시성 3으로 부분 리뷰를 실행하고, 정책·변경 파일 inventory·정제된 부분 결과와 제한된 PR head 근거를 최대 300,000자 통합 컨텍스트로 한 번 더 검토한다. 통합 호출에는 일반 구현 원문 diff를 다시 포함하지 않는다.
- 부분 리뷰는 일반적으로 최대 3개, 최종 결과는 최대 5개의 finding을 낸다. 다만 부분 리뷰의 `critical`과 명시적으로 기각되지 않은 `high`는 이 게시 상한을 넘어도 보존한다. `high`는 성공한 최종 통합이 전체 파일·동일 PR 정책·다른 chunk 근거로 오탐이나 중복임을 입증하고 `dismissed_findings`에 정확한 `root_cause`와 이유를 기록할 때만 제외한다. 통합 실패 fallback도 이들을 보존한다.
...
리뷰 결과는 같은 head SHA를 표시하는 PR sticky comment와 `PR Policy Agent` Check에 기록된다. sticky comment에는 보존된 finding 표와 최대 5개의 상세, 최종 교차 검증에서 제외한 부분 finding의 bounded 감사 요약을 남긴다. Discord에는 PR·Check·Actions 링크가 있는 요약과 최대 5개의 정제된 finding 상세, 단일·분할·증분 재사용 방식과 모델·시간·합산 token·cache read/write 사용량을 순서대로 보낸다. `allowed_mentions.parse=[]`로 멘션을 막고 메시지는 1,800자 이하로 분할한다.
