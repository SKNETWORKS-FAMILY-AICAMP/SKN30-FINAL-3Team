...
약 10명 시연 기준으로 전용 감시 Lambda·주기 실행·8개 RunPod 경보를 제거했다.
미종료·상태 불일치 자동 알림은 없으며 기존 Backend·AI 오류 알림은 유지한다.
GPU를 생성한 운영자가 시작·종료와 사용액을 직접 확인한다.
복구는 [RunPod runbook](../../infra/runpod/README.md)의 offline 정리·정확한 Pod ID 삭제·reconcile을 따른다.
AWS 쓰기 또는 refresh 실패는 복구 미완료이므로 정상이라고 보고하지 않는다.
API key·presigned URL·외부 오류 본문을 로그나 Discord에 복사하지 않는다.
