...
아래 책임 분리와 제한을 1차 구현에 적용했다. 실제 평가 상태는 [구현·검증](implementation-and-validation.md)을 따른다. 기존 [ADR-0006](../../../.agents/skills/project-wiki/references/decisions/ADR-0006-ai-backend-boundary.md)의 모듈 경계를 유지한다.
...
- 초기 Luna 검증 후 사용자 승인으로 [Qwen 3모델 비교](../../../infra/serving/model-comparison-2026-09-08.md)를 진행했다. 프로필·실제 결과·기동 실패·적용 한계는 비교 기록이 정본이다.
