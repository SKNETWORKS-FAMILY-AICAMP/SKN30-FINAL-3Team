...
- 초기 Luna 검증 후 사용자 승인으로 [Qwen 3모델 비교](../../../infra/serving/model-comparison-2026-09-08.md)를 진행했다. 프로필·실제 결과·기동 실패·적용 한계는 비교 기록이 정본이다. Luna·Qwen 점수는 workflow v2·v3 및 scorer v2 이전 실행이며, 이번 병합 검사는 수정 후 실제 모델 평가가 아니다. 공식 FP8 선택과 기존 활성 모델의 명시적 변경 경계를 유지한다.
