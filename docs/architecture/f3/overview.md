...
> 사용자의 저장 또는 상세의 판정 실행 버튼에 반응해 Backend가 복구 가능한 F3 작업을 만들고, AI가 격리된 양측 대리와 Backend 조회 capability를 사용해 후보를 판정하며, Backend가 검증한 최종 결과를 F1을 막지 않는 패널에 표시한다.

### 포함 범위

- 매물·손님 저장 및 상세의 판정 실행 버튼(세대 상세는 [교차 판정 실행], 손님 상세는 [교차 판정])에서 시작하는 핵심 교차 판정
...
|---|---|---|
| Frontend | 상세의 판정 실행 버튼 요청, 진행 구독, 단계 결과·최종 판정·근거·피드백 표시. 세대 상세의 레일 [교차 판정]은 섹션 여닫기 전용이며 실행을 요청하지 않는다 | SQL·검색, Agent 실행, 결과 영속화 |
