...
> **구현 메모 · 2026-09-16:** 모델이 정식 필드명 대신 다른 이름을 붙이는 경우
> (예: 매수문의에서 이름·전화를 매물장 라벨 `임대인 이름`·`임대인 전화`로 출력)
> 허용 검사 전에 `ai/src/brokerage_ai/f2/prompts.py`의 `FIELD_ALIASES`로 장부별
> 정식 필드명으로 되돌린다. 정규화한 제안은 `필드명을 정규화했습니다: A → B`
> 불확실 메모로 사용자에게 드러낸다. 매물장에서 역할이 갈리는 `전화번호`·`이름`은
> 임대인·임차인을 구분할 수 없어 매핑하지 않고 계속 제외한다.
> 근거 대조는 공백과 구두점·하이픈을 뺀 글자만 비교한다. STT 마지막 문장에 마침표가
> 없는데 모델 근거에는 붙어 있어 이름·전화가 제외되던 사례(2026-09-16, RunPod
> `consultation-v3` 실측)를 막기 위한 것이며, 근거가 원문의 연속 구간이어야 한다는
> 조건은 유지한다.
