...
- 2026-09-09: 사용자 1차 구현 요청에 따라 ADR-0035로 F3 조건부 자동 판정·완료 재사용·저장 결과 GET을 구현했다. 최신 dev f700f5a 병합, 원자 outbox와 Worker 복구, 기존 디자인 시스템의 결과 목록, 합성 테스트 모델 통합 검증을 반영했다. 실제 모델 품질·공유 dev 배포는 검증 범위에서 제외한다.

- 2026-09-09: 사용자 요청으로 자동 판정 상태 3테이블을 match 도메인 명명으로 변경하고 021 전진 rename migration, SQL·테스트·문서를 함께 갱신했다.

- 2026-09-09: 사용자 후속 요청으로 합성 장부 시드에 결정적 예시 매칭 결과·출처 표시와 결과 검증을 추가했다. 기존 로컬 DB 적용·시드 로그인 검증 및 공유 dev 고정 명령 확장은 [시드 안내](../../../../docs/db/seed/README.md)를 따른다.

- 2026-09-09: 사용자 요청에 따라 F3 확장 문서와 구현을 별도 연계 PR로 분리하고, 자동 접수 ENABLED의 독립 운영 목적·설정 범위를 문서화했다. 환경변수 주석은 한글로 정리했다.

- 2026-09-09: PR 리뷰를 대조해 사무소당 1행 outbox의 배치·잠금 경계와 새 조회 API의 표시명 범위를 명시했다. 과거 후보의 현재 적격성과 snapshot 포함 여부를 분리하고 삭제·권한 제한은 유지한다.
