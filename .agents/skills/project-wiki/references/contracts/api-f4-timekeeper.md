...
인증·오류의 공통 규칙은 [API 공통 계약](api-common.md)을 함께 읽는다. 직접 저장하는 일정은 [F4 캘린더 계약](api-f4-calendar.md)이 소유한다.
...
**여덟 번째 갈래는 캘린더 일정이다.** 사용자가 `/calendar/events`([F4 캘린더 일정 계약](api-f4-calendar.md))로 직접 만든 일정도 이 응답에 함께 실린다. 이 갈래는 위 표의 고정 값이 아니라 사용자가 고른 임의
