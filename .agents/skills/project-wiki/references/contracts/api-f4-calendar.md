...
> **[폐기 · 2026-09-08]** 캘린더 기능 전체를 사용자 요청으로 폐기했다
> ([ADR-0038](../decisions/ADR-0038-remove-calendar-feature.md)). 아래는 폐기 시점까지 구현됐던
> 계약을 과거 기록으로 보존한 것이며 현재 어떤 경로도 존재하지 않는다.

인증·오류의 공통 규칙은 [API 공통 계약](api-common.md)을 함께 읽는다. Time Keeper 통합도 함께
되돌렸다 — [Time Keeper 계약](api-f4-timekeeper.md)은 장부 파생 갈래만 남는다.

## F4 캘린더 일정 계약 (폐기 전 · 제안 상태로 종료)
