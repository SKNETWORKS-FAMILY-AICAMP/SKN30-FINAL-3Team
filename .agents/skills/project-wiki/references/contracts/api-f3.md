...
인증·오류의 공통 규칙은 [API 공통 계약](api-common.md)을 함께 읽는다. AI 입력·결과를 변경할 때는 [F3 Backend–AI 계약](f3-ai.md)에서 해당 계약을 선택한다.
...
앵커 카드에 `invalidated_at`이 있거나 참조 카드가 없으면 실행 상태는 유지하되 카드·후보·판정·
근거 결과는 빈 값으로 반환한다. 판정 헤더가 있으면 그 앵커 참조를 사용하고, 무효 헤더 카드를
다른 실행 snapshot 카드로 대체하지 않는다. 헤더가 없는 단계에서만 snapshot 참조를 사용한다.
이는 명시적 무효화의 조회 경계이며 모든 장부 변경에 대한 결과 최신성 검증을 의미하지 않는다.
...
### 브라우저 조회 재개 (2026-09-10)

조회 일시 중단·통신 실패의 `다시 확인`은 메모리에 확보한 동일 run_id로 GET과 polling만 재개한다.
기존 실행이 404여도 자동 POST하지 않으며 사용자에게 명시적 재판정을 제공한다. 최초 요청과
명시적 재판정만 POST하며 기존 Backend 활성 실행 중복 방지 정책을 따른다.
결과 GET의 최신 status를 화면에 반영하고 terminal이면 즉시 polling을 끝낸다. 계정 변경·패널
닫기 뒤 도착한 응답은 현재 화면과 실행 캐시에 반영하지 않는다. 브라우저 영구 저장소에 결과나
실행 ID를 추가하지 않으며 새로고침을 넘는 완료 재사용 기능도 추가하지 않는다.
HTTP DTO·권한·CSRF·후보 순서·페이지 계약은 변경하지 않는다.

2026-09-10 후속 사용자 지시에 따라 자동 polling 대기 상한을 60초에서 **300초**로 늘렸다.
60초를 넘겨도 같은 실행을 계속 조회하며 300초 후의 수동 재개도 GET만 사용한다.
이 시간은 Frontend 조회 세션의 상한이며 Backend 실행을 취소하거나 모델 호출을 다시 만드는 시간이 아니다.
