...
PR #99의 조건 검증 보완은 `chatbot-workflow:v2`에 도입됐고, 현재 v4에도 유지된다.
...
`chatbot-workflow:v4`·`chatbot-prompt:v2`는 일정 종류의 만기/만료 동의어를 함께 인식한다.
세대·매물 임대차, 고객 임대차, 구입장의 대상 근거를 유지하며 `만기`만으로 특정 종류를
임의 선택하지 않는다. v3의 고정 되먹임·호출 상한은 유지한다. 합성 Provider 검증은 실제
모델 해석 성공률을 보장하지 않으며, 이전 실행 결과는 당시 버전 그대로 보존한다.
