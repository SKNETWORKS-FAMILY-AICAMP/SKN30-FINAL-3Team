...
| 이미지 | ECR | 결정 | 적용됨 | immutable tag, untagged image만 7일 후 만료; 배포는 digest 고정 |
...
| 전달 | CodeConnections, CodePipeline V2, CodeBuild, CodeDeploy | 결정 | 코드 구현됨·미적용 | 통합 main 자동, Backend·Frontend 수동, 모두 QUEUED |
...
- delivery Terraform은 세 Pipeline, CodeBuild/CodeDeploy, Discord 알림과 기존 IAM 운영자 policy attachment까지 코드로 구현했지만 plan 승인 전에는 적용하지 않는다.
