...
| 관측성 | CloudWatch logs·metrics·alarms, SNS·Lambda | 결정 | 기존 6개 alarm 적용·전용 전달과 2개 app alarm 코드 구현, AWS 미적용 | RunPod 자체 감시는 제거하고 운영자가 시작·종료 확인; 기존 Backend·AI 오류 알림 유지 |
...
| 모델 설정 | RunPod Secret, AWS Secrets Manager, SSM endpoint/control set | 결정 | 코드 구현·AWS/RunPod 미적용 | 별도 SLLM·STT key, active/offline 두 URL, Console Template·registry ID와 image digest 단일 등록; Infra ADR-0020 적용 |
...
- RunPod는 Terraform provider 대신 Git Template 명세와 Console 자원 검증·등록 및 기본 dry-run reconcile,
...
## AWS·RunPod GPU 통합 추가

[ADR-0022](decisions/ADR-0022-dual-cloud-gpu-lifecycle.md)의 GPU EC2·gp3·SSM 터널·범용 Template 등록 코드가 추가됐다. GPU 생성 기본값은 빈 집합이고 실제 GPU 추론·왕복 전환은 미검증이다. 기존 RunPod 등록값은 보존한다.

2026-09-07 기반 Terraform 적용: general 등록·endpoint·selection과 고정 SSM 문서·팀 그룹을 생성하고 RunPod 감시 자원을 제거했다. 앱 ASG 0·RDS stopped·관리 GPU 0·CloudFront disabled를 유지했으며 기존 F2 등록과 업무 S3 3개를 보존했다. 동일 중지 입력의 drift는 없다.
