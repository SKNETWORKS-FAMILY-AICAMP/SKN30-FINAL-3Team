...
class ObservabilityContractTests(unittest.TestCase):
    def test_delivery_notifier_route_is_preserved_and_alarm_route_is_separate(
        ...
        self.assertIn(
            "topic_arn = aws_sns_topic.runtime_alerts.arn", delivery
        )
        self.assertIn(
        ...
        self.assertIn(
            "topic_arn = aws_sns_topic.cloudwatch_alarms.arn", observability
        )
        self.assertNotIn(
            "aws_sns_topic.runtime_alerts.arn]", observability.split(
                'resource "aws_cloudwatch_metric_alarm"', maxsplit=1
            )[1],
        )
    ...
    def test_alarm_webhook_is_an_independent_write_only_secret(self) -> None:
        ...
        self.assertNotIn(
            "aws_secretsmanager_secret.discord_webhook.arn", observability
        )
        notifier = section(
            observability,
    ...
    def test_cloudwatch_topic_policy_limits_alarm_publishers(self) -> None:
        ...
        self.assertIn(
            "alarm:${local.name_prefix}-*", topic_policy
        )
    ...
    def test_backend_and_ai_filters_publish_dimensionless_count_metrics(self) -> None:
        ...
        self.assertIn('pattern        = "{ $.event = \\"unhandled_request_error\\" }"', backend_filter)
        self.assertIn('log_group_name = aws_cloudwatch_log_group.runtime["api"].name', backend_filter)
        for filter_config, group in ((ai_api_filter, "api"), (ai_worker_filter, "worker")):
            self.assertIn('pattern        = "{ $.event = \\"ai_terminal_failure\\" }"', filter_config)
            self.assertIn(
