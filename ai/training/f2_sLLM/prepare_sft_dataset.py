...
def convert_full_sample(sample: dict[str, Any], source: str) -> dict[str, Any]:
    ...
    if not isinstance(expected, dict):
        raise ValueError(f"{source}: expected는 JSON 객체여야 합니다")
    missing_expected = FULL_EXPECTED_KEYS - expected.keys()
    if missing_expected:
        raise ValueError(f"{source}: expected 필수 필드 누락 {sorted(missing_expected)}")
    unexpected_expected = expected.keys() - FULL_EXPECTED_KEYS
    if unexpected_expected:
        raise ValueError(f"{source}: expected 알 수 없는 필드 {sorted(unexpected_expected)}")
    ...
    if (expected["ledger_mismatch"] or sample["label"] == "기타상담") and (
        expected["fields"] or expected["evidence"]
    ):
