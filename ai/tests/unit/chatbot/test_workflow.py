...
async def test_category_list_validates_every_member_and_can_repair():
    provider = FakeProvider(
        {"tool": "agenda", "filters": {"categories": ["LISTING_REVALIDATION", "MOVE_IN"]}},
        {"tool": "agenda", "filters": {"categories": ["LISTING_REVALIDATION"]}},
    )
    reads = FakeReads()
    await workflow(provider).run(request("매물 재확인 일정"), capability=reads)
    assert len(provider.requests) == 2
    assert reads.calls[0][0].filters.categories == ("LISTING_REVALIDATION",)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("그중 입주만", "agenda", {"categories": ["LISTING_REVALIDATION"]}),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("매물 재확인 제외하고 입주 일정", "agenda", {"categories": ["LISTING_REVALIDATION"]}),
    ],
)
