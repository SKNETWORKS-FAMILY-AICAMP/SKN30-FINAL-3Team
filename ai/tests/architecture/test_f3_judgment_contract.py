"""중개 판정 코드와 project-wiki의 구현 범위를 함께 고정한다."""

from pathlib import Path

REPOSITORY_ROOT = Path(__file__).resolve().parents[3]


def test_judgment_contract_and_project_wiki_stay_aligned() -> None:
    references = REPOSITORY_ROOT / ".agents" / "skills" / "project-wiki" / "references"
    contract = (references / "contracts" / "f3-ai-brokerage.md").read_text()
    api_contract = (references / "contracts" / "api-f3.md").read_text()
    implementation = (references / "contracts" / "f3-ai-implementation.md").read_text()
    router = (references / "contracts" / "f3-ai.md").read_text()
    index = (references / "index.md").read_text()
    log = (references / "log.md").read_text()
    online_runtime = (
        REPOSITORY_ROOT / "docs" / "architecture" / "f3" / "online-runtime.md"
    ).read_text()

    assert "# F3 중개 판정 Backend–AI 계약" in contract
    assert "`brokerage-judgment:v1`" in contract
    assert "결과를 반환하기 전에 직접 호출" in contract
    assert "`SYNTHETIC_PROTOTYPE` 요청은 생성기 조립 지점" in contract
    assert "저장된 앵커·후보 카드의 판정 요청 조립" in implementation
    assert "| `JUDGING` | 업무 처리 | 전체 후보 중개 판정 실행 중 | 구현됨 |" in api_contract
    assert "| `COMPLETED` | 업무 처리 | 검증을 통과한 최종 결과 저장 | 구현됨 |" in api_contract
    assert "](contracts/f3-ai.md)" in index
    assert "](f3-ai-brokerage.md)" in router
    assert "F3 중개 판정 Backend–AI 계약" in log
    assert "중개 판정 Backend–AI 공개 계약과 구조화 출력 생성" in online_runtime
    assert "중개 판정 요청 조립, 결과·근거 저장과 `JUDGING`·`COMPLETED` 전이" in online_runtime
