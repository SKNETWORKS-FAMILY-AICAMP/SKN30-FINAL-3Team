...
def test_verification_requires_exactly_29_passes() -> None:
    validate_verification_rows(passing_rows())

    with pytest.raises(SyntheticSeedError, match="expected 29 checks"):
...
def test_seed_requires_explicit_reset_confirmation(make_config) -> None:
    ...
    with pytest.raises(SyntheticSeedError, match="--confirm-reset"):
        seed_f3_synthetic(config, confirm_reset=False)


def test_manage_exposes_local_f3_synthetic_seed_command() -> None:
    arguments = manage.build_parser().parse_args(["seed-f3-synthetic", "--confirm-reset"])
...
def test_manage_prints_seed_identity_as_json(
    ...
    def run_seed(resolved_config, *, confirm_reset: bool):
        calls.append((resolved_config, confirm_reset))
    ...
    monkeypatch.setattr(
        sys,
        "argv",
        ["manage.py", "seed-f3-synthetic", "--confirm-reset"],
    )
