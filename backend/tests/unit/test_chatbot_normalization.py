...
def test_saved_retired_agenda_page_clarifies_before_database(offset, monkeypatch):
    ...
    result = query.ChatLookup(Mock(), 1).search(
        {
            "tool": "agenda",
            "categories": ["CALENDAR", "LISTING_RECONTACT"],
...
def test_supported_calendar_refinement_replaces_retired_scope():
    filters = normalize(
        ChatIntent(tool="agenda", mode="refine", filters=ChatFilters(categories=("CALENDAR",))),
        ChatInput(
            question="캘린더 일정으로",
            as_of=date(2026, 9, 8),
            active_filters={
    ...
    assert filters["categories"] == ["CALENDAR"]
...
def test_calendar_free_text_category_is_not_a_retired_ledger_capability(monkeypatch):
    session_factory = MagicMock()
    session = session_factory.return_value.__enter__.return_value
    session.execute.return_value.first.return_value = (7,)
    monkeypatch.setattr(query, "Session", session_factory)
    reference = ResultReference(
        kind="agenda",
        items=(
            ChatResultItem(
                id="CLIENT_RECONTACT:None:None:None:7",
                title="사용자가 직접 만든 일정",
                action=ChatAction(type="open_calendar", target_id=7, label="캘린더 보기"),
            ),
        ),
    )
    result = asyncio.run(
    ...
    assert result.actions[0].type == "open_calendar"
    assert result.actions[0].target_id == 7
