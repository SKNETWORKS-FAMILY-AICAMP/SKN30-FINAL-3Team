...
def test_disabled_automation_leaves_worker_body_running_without_consuming_events(monkeypatch):
    from domain.agent_execution import automation

    tick = MagicMock()
    session_factory = MagicMock()
    monkeypatch.setattr(automation, "tick", tick)
    monkeypatch.setattr(reliability, "Session", session_factory)
    worker_body = MagicMock()
    with reliability.maintain_automation(MagicMock(spec=Engine), threading.Event(), enabled=False):
        worker_body()
    worker_body.assert_called_once()
