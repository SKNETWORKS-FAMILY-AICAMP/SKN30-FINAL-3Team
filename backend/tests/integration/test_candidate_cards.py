...
@requires_database
def test_an_anchor_changed_during_candidate_generation_does_not_advance_the_run() -> None:
    """후보 카드 도중 앵커가 바뀌면 그 후보 집합을 완료 처리하지 않는다."""
    with db_session() as session:
        fixture = Fixture(session)
        fixture.requirement(budget=3_000_000_000)
        run_id = fixture.prepared_run()

        class ChangingAnchorGenerator(FakeGenerator):
            async def generate_position_card(
                self, request: PositionCardGenerationRequest
            ) -> PositionCardGenerationResult:
                produced = await super().generate_position_card(request)
                with db_session() as other:
                    other.execute(
                        text(
                            "UPDATE property_listing SET row_version = row_version + 1"
                            " WHERE id = :i"
                        ),
        ...
        with pytest.raises(InputVersionChangedError):
            asyncio.run(
                generate_and_store_candidate_cards(
                    ...
                    worker_id=WORKER,
                    attempt_count=ATTEMPT,
                    binding=binding_for(fixture, ChangingAnchorGenerator()),
                    as_of=AS_OF,
                )
            )
