...
class ChatLookup:
    ...
    def _agenda(self, session, filters, offset):
        ...
        if categories:
            conditions = []
            ledger_categories = [category for category in categories if category != "CALENDAR"]
            if ledger_categories:
                conditions.append(combined.c.category.in_(ledger_categories))
            if "CALENDAR" in categories:
                conditions.append(combined.c.event_id.is_not(None))
        ...
        rows = session.execute(
            query.order_by(
                ...
                combined.c.event_id.asc().nullslast(),
            )
            .limit(10)
            .offset(offset)
        ).mappings()
        ...
        for row in rows:
            if row["event_id"]:
                action = ChatAction(
                    type="open_calendar", target_id=row["event_id"], label="캘린더 보기"
                )
            elif row["unit_id"]:
                action = ChatAction(
            ...
            title = row["title"] or CATEGORY_LABELS.get(row["category"], row["category"])
            items.append(
                ChatResultItem(
                    id=f"{row['category']}:{row['unit_id']}:{row['listing_id']}:{row['requirement_id']}:{row['event_id']}",
                    title=str(title),
