...
class GenerationBinding:
    ...
    `input_privacy_mode`도 조립 지점이 명시한다. 이번 슬라이스는 ADR-0014의 검토된 합성
    케이스만 허용하며 `MASKED` 실사용 입력 조립은 후속 작업이다. 생성기의 별도 synthetic
    opt-in은 AI 모듈이 독립적으로 검사한다.
    """
...
def _require_supported_privacy_mode(mode: InputPrivacyMode) -> None:
    if mode is not InputPrivacyMode.SYNTHETIC_PROTOTYPE:
        raise GenerationBindingError("masked F1 snapshot assembly is not implemented")
    ...
    *,
    ...
    try:
        ...
        prepared = PreparedGeneration(
            ...
            input_privacy_mode=request.input_privacy_mode,
            cached_analysis_id=cached.id if cached else None,
            # cache hit 이면 Provider 전달 본문을 들고 다니지 않는다. 신원과 범위만 남긴다.
    ...
    for field_name, evidence in grouped:
        for order, item in enumerate(evidence):
            ...
            if item.kind is EvidenceKind.QUOTE and item.quote_text is not None:
                # 합성 프로토타입은 본문을 변환하지 않으므로 Provider 입력과 원문의 위치가 같다.
