...
class JudgmentResultMismatchError(RuntimeError):
    """후보 유무와 판정 결과 유무가 맞지 않는다."""
...
def _judgment_snapshot(binding: JudgmentBinding, model_snapshot: dict[str, object]) -> dict:
    ...
    return {
        ...
        "input_privacy_mode": binding.input_privacy_mode.value,
    }


def _require_supported_privacy_mode(mode: InputPrivacyMode) -> None:
    """현재 Backend 조립은 ADR-0014의 합성 프로토타입 입력만 지원한다."""
    if mode is not InputPrivacyMode.SYNTHETIC_PROTOTYPE:
        raise GenerationBindingError("masked F1 judgment assembly is not implemented")
...
def _position_card_target(card: NegotiationPositionAnalysis) -> tuple[AnchorType, int]:
    """카드가 가리키는 장부 대상을 복원한다. 측면과 FK가 정확히 맞아야 한다."""
    if (
        card.negotiation_side == AnchorType.LISTING.value
        and card.listing_id is not None
        and card.requirement_id is None
    ):
        return AnchorType.LISTING, card.listing_id
    if (
        card.negotiation_side == AnchorType.REQUIREMENT.value
        and card.requirement_id is not None
        and card.listing_id is None
    ):
        return AnchorType.REQUIREMENT, card.requirement_id
    raise AnchorCardMissingError("a position card has an invalid target")
...
def prepare_judgment(
    ...
    try:
        ...
        run = repository.find_leased_run(
            ...
            worker_id,
            attempt_count,
            status=(CANDIDATE_CARDS_READY_STATUS, JUDGING_STATUS),
        )
        if run is None:
        ...
        if candidate_card_ids:
            ...
            if run.status == CANDIDATE_CARDS_READY_STATUS:
                ...
                snapshot["judgment"] = expected_binding
...
def store_judgment(
    ...
    try:
        if empty and result is not None:
            raise JudgmentResultMismatchError("an empty candidate set must not carry a result")
        if not empty and result is None:
            raise JudgmentResultMismatchError("a non-empty candidate set requires a result")
        ...
        if not empty:
            ...
            if prepared.request.input_privacy_mode is not binding.input_privacy_mode:
                raise GenerationBindingError("the input privacy mode changed while judging")
            if (
                _expected_model_snapshot(session, run.brokerage_id, binding)
