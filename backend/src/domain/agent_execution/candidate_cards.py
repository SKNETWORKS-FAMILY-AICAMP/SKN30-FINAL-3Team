...
마스킹, cache key, 저장 직전 재검증과 개인정보 검사를 다시 구현하지 않는다.
...
하나가 곧 transaction 하나라 세션을 나누면 커넥션 수와 fencing 이 함께 복잡해진다. 카드는
대개 캐시에서 나오므로(F3-PC-12, F3-NF-03) 실제 모델 호출은 평시 1~3회로 수렴한다.

ponytail: 제한 병렬화는 세션 분리가 먼저 정리된 뒤에 넣는다. 지금 넣으면 얻는 지연 감소보다
공유 세션 버그 위험이 크다.
"""
...
def _plan_from_snapshot(run: AgentRun, header: repository.MatchEvaluation) -> CandidateCardPlan:
    ...
    for entry in entries:
        if not isinstance(entry, dict) or not entry.get("selected_for_cards"):
            continue
