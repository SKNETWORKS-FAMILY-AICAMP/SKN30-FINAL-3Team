...
from domain.authentication.models import AppUser
...
def list_party_identifiers(
    session: Session, brokerage_id: int, party_ids: Sequence[int]
) -> list[str]:
    """마스킹해야 할 인물 식별값. 이 값들은 AI 요청에 그대로 실리면 안 된다."""
    if not party_ids:
        return []
    names = session.execute(
        select(col(Party.name), col(Party.alternate_name)).where(
            col(Party.brokerage_id) == brokerage_id,
            col(Party.id).in_(party_ids),
        )
    ).all()
    contacts = session.execute(
        select(col(PartyContact.contact_value), col(PartyContact.normalized_contact_value)).where(
            col(PartyContact.brokerage_id) == brokerage_id,
            col(PartyContact.party_id).in_(party_ids),
        )
    ).all()
    values = [value for row in (*names, *contacts) for value in row if value]
    return values


def list_user_identifiers(
    session: Session, brokerage_id: int, user_ids: Sequence[int | None]
) -> list[str]:
    """담당자와 요청자의 로그인 ID·표시 이름. 상담 원문에 섞여 있을 수 있다."""
    identifiers = [user_id for user_id in user_ids if user_id is not None]
    if not identifiers:
        return []
    rows = session.execute(
        select(col(AppUser.login_id), col(AppUser.display_name)).where(
            col(AppUser.brokerage_id) == brokerage_id,
            col(AppUser.id).in_(identifiers),
        )
    ).all()
