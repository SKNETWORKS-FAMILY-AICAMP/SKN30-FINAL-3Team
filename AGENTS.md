...
- 모듈 내부 구조는 해당 모듈의 승인된 결정과 현재 코드를 확인하며, 필요하지 않은 구조를 미리 확정하지 않는다.
- 개발 전에 목적·복잡도·변경 범위를 판단하고, 기존 계약과 승인된 결정을 지키는 가장 단순한 아키텍처를 선택한다.
- 수작업으로 유지하는 파일이 1,000줄을 넘거나 이번 변경으로 1,000줄 이상 늘어나면 책임 단위로 분리한다. 자동 생성 파일, lockfile, 원문 스냅샷은 제외한다.
- Python은 목적에 맞게 객체지향·함수 중심·절차적 구성을 선택하거나 조합한다. 불필요한 클래스·계층·추상화를 만들지 않는다.
...
2. 현재 작업과 직접 관련된 문서만 읽는다. 링크된 문서를 재귀적으로 전부 읽지 않는다.
...
현재 컨텍스트에 내용이 있고 이후 파일이 바뀌지 않은 지침은 다시 읽지 않는다. 문서 내용이 없거나 변경 여부가 불확실하면 필요한 부분을 다시 확인한다.
