...
test("team defaults leave optional sources absent and inherit the selected ledger", () => {
  ...
  delete env.VITE_CALENDAR_SOURCE;
    ...
    [
      ...
      `const { parseAppEnv } = await import(${JSON.stringify(schemaUrl)});
       ...
       console.log(JSON.stringify([
         ...
         Object.hasOwn(process.env, "VITE_CALENDAR_SOURCE"),
         parsed.f3Source, parsed.calendarSource
       ]));`,
    ],
    { encoding: "utf8", env },
  );
  assertSpawned(result);
  assert.deepEqual(JSON.parse(result.stdout), [false, false, "api", "api"]);
});
