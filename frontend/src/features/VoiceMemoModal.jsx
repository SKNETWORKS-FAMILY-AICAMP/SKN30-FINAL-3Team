...
export default function VoiceMemoModal({ isOpen, draft, initialDraft, ledgerType = "property", appendTo, onClose, onApply, onDraftChange }) {
  ...
  const body = state === "empty" ? (
    <div className="f2-modal__empty">{renderUpload()}</div>
  ) : state === "sourceReady" ? (
    ...
    <div className="f2-processing" aria-live="polite"><div className="f2-processing__heading"><div><strong>음성메모를 분석하고 있습니다</strong></div><Button variant="secondary" icon={<TimesIcon />} onClick={cancelAnalysis}>분석 취소</Button></div><ProgressStepper isCenterAligned>{STEPS.map((item, index) => <ProgressStep key={item} variant={index < step ? "success" : index === step ? "info" : "pending"} isCurrent={index === step}>{item}</ProgressStep>)}</ProgressStepper></div>
  ) : state === "error" ? (
  ...
  return <Modal id="f2-modal" isOpen={isOpen} onClose={requestClose} appendTo={appendTo} variant="large" aria-label={isIntake ? "음성메모 신규 접수" : "음성메모·AI 제안 검토"} className="voice-memo-modal"><ModalHeader title={isIntake ? "음성메모 신규 접수" : "음성메모·AI 제안 검토"} description={isIntake ? "음성을 분석해 매도의뢰는 매물장, 매수문의는 구입장 신규 행으로 추가합니다. 저장은 상세에서 확인한 뒤 진행합니다." : "F1 상세 위에서 파일을 분석하고 선택한 제안만 작성값에 반영합니다."} /><ModalBody><div className="voice-memo-modal__content" data-screen-id="F2-MOD-010" data-requirement-ids="F1-ST-01~05, F1-ST-06~11, F1-ST-12~15, F1-ST-15~18, F2-LIST-01~04, F2-POP-03, F2-REV-01~04">{body}<Alert variant="warning" isInline title="개인정보 포함 음성 주의"><div className="f2-privacy-copy">주민등록번호, 계좌번호, 비밀번호가 포함된 음성은 업로드하지 마세요.</div><Checkbox id="f2-privacy-confirm" label="주의 문구를 확인했으며 상담 음성 파일만 사용합니다." isChecked={confirmed} onChange={(_event, checked) => setConfirmed(checked)} /></Alert>{state === "sourceReady" && <div className="f2-actions f2-actions--end"><Button variant="primary" icon={<PlayIcon />} isDisabled={!confirmed || !file} onClick={startAnalysis}>분석 시작</Button></div>}</div></ModalBody><ModalFooter><Button variant="link" onClick={requestClose}>닫기</Button></ModalFooter></Modal>;
}
